...
import com.qumasoft.guitools.qwin.OperationVisualMerge;
...
public class MergeFrame extends javax.swing.JFrame {
    ...
    /**
     * Deduce the EOL sequence for this file. The algorithm traverses the lines of the file (as captured in the linked list passed as an argument), and examines the number of
     * characters that are used as the EOL sequence. If the same number of characters are always used (either 1 or 2), then we can conclude the the EOL sequence is consistent
     * throughout the file, and we can use that sequence. If the EOL sequence varies, then we'll use the current platform's EOL sequence.
     *
...
// <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    ...
    private void computeEditScript(String baseFileName, String decendentFileName, TreeMap<String, EditInfo> editScript) throws IOException, QVCSOperationException {
        ...
        LOGGER.log(Level.INFO, "Comparing " + baseFileName + " to " + decendentFileName);
    ...
    private void applyDecendentEdits(LinkedList<DescendentFileContentRow> decendentFileLinkedList, TreeMap<String, EditInfo> editScript) throws QVCSOperationException {
        Iterator<EditInfo> editInfoIterator = editScript.values().iterator();
        ListIterator<DescendentFileContentRow> rowIterator = decendentFileLinkedList.listIterator();
        int decendentLineNumber = 1;
        ...
        while (editInfoIterator.hasNext()) {
            ...
            while (rowIterator.hasNext()) {
                ...
                if (existingRowStartPosition < editSeekPosition) {
                    existingRow.setDecendentLineNumber(decendentLineNumber++);
                    continue;
                }
                break;
            }
            ...
            switch (editInfo.getEditType()) {
                case CompareFilesEditInformation.QVCS_EDIT_DELETE: {
                    ...
                    int rowCounter = 0;
                    do {
                        existingRow.setDecendentRowType(MergedDescendentFileContentRow.ROWTYPE_DELETE);
                        existingRow.setDecendentEditInfo(editInfo);
                        if (rowCounter == 0) {
                            existingRow.setDecendentFirstRowOfEditFlag(true);
                        }
                        existingRow = rowIterator.next();
                ...
                case CompareFilesEditInformation.QVCS_EDIT_INSERT: {
                    ...
                    for (int i = 0; i < insertionText.length; i++, rowCounter++) {
                        ...
                        newRow.setDecendentLineNumber(decendentLineNumber++);
                        newRow.setDecendentRowType(MergedDescendentFileContentRow.ROWTYPE_INSERT);
                        newRow.setDecendentEditInfo(editInfo);
                        newRow.setDecendentText(insertionText[i]);
                        if (rowCounter == 0) {
                            newRow.setDecendentFirstRowOfEditFlag(true);
                        }
                ...
                case CompareFilesEditInformation.QVCS_EDIT_REPLACE: {
                    ...
                    do {
                        ...
                        existingRow.setDecendentLineNumber(0);
                        existingRow = rowIterator.next();
                        if (existingRow.getAncestorSeekPosition() >= endOfEditSeekPosition) {
                            rowIterator.previous();
                            break;
                    ...
                    for (String insertionText1 : insertionText) {
                        ...
                        newRow.setDecendentRowType(MergedDescendentFileContentRow.ROWTYPE_INSERT);
                        newRow.setDecendentEditInfo(editInfo);
                        newRow.setDecendentText(insertionText1);
                        rowIterator.add(newRow);
                    }
                    break;
                }
    ...
    private LinkedList<MergedDescendentFileContentRow> populateFilePanels(final String baseFileName) {
        ...
        try {
            ...
            if (populateBaseFileLinkedList(baseFileName, baseFileLinkedList)) {
                ...
                applyDecendentEdits(firstDecendentLinkedList, firstDecendentEditScript);

                // Apply the edits from the second decendent...
                populateDecendentLinkedList(baseFileLinkedList, secondDecendentLinkedList);
                applyDecendentEdits(secondDecendentLinkedList, secondDecendentEditScript);

                // Merge the edits from the first and second decendents to the contents of
                // the ancestor file.
                mergeDecendentEdits(baseFileLinkedList, firstDecendentLinkedList, secondDecendentLinkedList);
                ...
                LOGGER.log(Level.WARNING, "Unable to read: '" + baseFileName + "'");
            }
    ...
    /**
     * Merge the two decendents into the parent base file.
     *
     * @param resultLinkedList
     * @param firstDecendentLinkedList
     * @param secondDecendentLinkedList
     */
    void mergeDecendentEdits(LinkedList<MergedDescendentFileContentRow> resultLinkedList,
                             LinkedList<DescendentFileContentRow> firstDecendentLinkedList,
                             LinkedList<DescendentFileContentRow> secondDecendentLinkedList) {
        mergeFirstDecendent(resultLinkedList, firstDecendentLinkedList);
        mergeSecondDecendent(resultLinkedList, secondDecendentLinkedList);
    }

    void mergeFirstDecendent(LinkedList<MergedDescendentFileContentRow> resultLinkedList,
                             LinkedList<DescendentFileContentRow> firstDecendentLinkedList) {
        ...
        try {
            ...
            while (resultIterator.hasNext() && firstDecendentIterator.hasNext()) {
                ...
                switch (decendentRow.getDecendentRowType()) {
                    case MergedDescendentFileContentRow.ROWTYPE_DELETE: {
                        ...
                        mergedRow.setFirstDecendentEditInfo(decendentRow.getDecendentEditInfo());
                        mergedRow.setFirstDecendentCheckBoxVisibleFlag(decendentRow.getDecendentFirstRowOfEditFlag());
                        break;
                    }
                    case MergedDescendentFileContentRow.ROWTYPE_INSERT: {
                        ...
                        do {
                            ...
                            newRow.setFirstDecendentEditInfo(decendentRow.getDecendentEditInfo());
                            newRow.setFirstDecendentCheckBoxVisibleFlag(decendentRow.getDecendentFirstRowOfEditFlag());
                            newRow.setFirstDecendentRowType(MergedDescendentFileContentRow.ROWTYPE_INSERT);
                            newRow.setFirstDecendentText(decendentRow.getDecendentText());
                        ...
                        } while (decendentRow.getDecendentRowType() == MergedDescendentFileContentRow.ROWTYPE_INSERT);
    ...
    void mergeSecondDecendent(LinkedList<MergedDescendentFileContentRow> resultLinkedList,
                              LinkedList<DescendentFileContentRow> secondDecendentLinkedList) {
        ListIterator<MergedDescendentFileContentRow> resultIterator = resultLinkedList.listIterator();
        ...
        try {
            ...
            while (resultIterator.hasNext() && secondDecendentIterator.hasNext()) {
                ...
                int secondDecendentRowType = decendentRow.getDecendentRowType();
                ...
                if ((secondDecendentRowType == MergedDescendentFileContentRow.ROWTYPE_DELETE)
                    ...
                    mergedRow.setSecondDecendentCheckBoxVisibleFlag(decendentRow.getDecendentFirstRowOfEditFlag());
                    mergedRow.setSecondDecendentEditInfo(decendentRow.getDecendentEditInfo());
                    mergedRow.setSecondDecendentText(decendentRow.getDecendentText());
                    ...
                    do {
                        MergedDescendentFileContentRow newRow = new MergedDescendentFileContentRow(frameFont);
                        newRow.setSecondDecendentCheckBoxVisibleFlag(decendentRow.getDecendentFirstRowOfEditFlag());
                        newRow.setSecondDecendentEditInfo(decendentRow.getDecendentEditInfo());
                        newRow.setSecondDecendentRowType(MergedDescendentFileContentRow.ROWTYPE_INSERT);
                        newRow.setSecondDecendentText(decendentRow.getDecendentText());
