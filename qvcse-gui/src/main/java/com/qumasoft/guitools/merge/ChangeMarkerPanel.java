...
public class ChangeMarkerPanel extends JPanel {
    private static final long serialVersionUID = 151859056428395878L;

//    private static final Logger m_logger = Logger.getLogger("com.qumasoft.guitools.merge");
    /**
     * Panel width in pixels.
     */
    private static final int PANEL_WIDTH = 10;
    private static final int PANEL_HEIGHT = 10;
    private Rectangle pastSize = null;
     ...
     * @param panelNumber this should be a 1 or a 2 for the first descendent or
     * @param rowHt the height of a single row. second descendent respectively.
     */
    ChangeMarkerPanel(LinkedList<MergedDescendentFileContentRow> baseFileLinkedList, int panelNumber, int rowHt) {
        ...
        assert (panelNumber == 1 || panelNumber == 2);
        this.rowHeight = rowHt;
        this.totalRowCount = computeRowBlocks(baseFileLinkedList, panelNumber);
        this.addMouseListener(new MyMouseListener(this));
    }

    /**
     * Set the scrollbar for this change panel so we can figure out what space to pre-allocate for the scroll bar buttons.
     *
    ...
    private void computeScrollButtonOffsets(Rectangle ourBoundingRectangle) {
        ...
        // Add more for the horizontal scroll bar that (almost) always appears at the bottom.
        // <editor-fold>
        scrollButtonTotalHeight += 15.0;
        scrollButtonBottomHeight += 15.0;
        // </editor-fold>
    }

    /**
     * Figure out the array of rectangles that we need to have to provide a summary of where the edits are in the given file.
     *
    ...
    private int translateRowType(int rowType) {
        ...
        switch (rowType) {
            case MergedDescendentFileContentRow.ROWTYPE_INSERT:
                translatedRowType = MergedDescendentFileContentRow.ROWTYPE_INSERT;
                break;
    ...
    class ColoredRowBlock {
        ...
        ColoredRowBlock(int rowType, int rc) {
            switch (rowType) {
                case MergedDescendentFileContentRow.ROWTYPE_DELETE:
                    this.color = ColorManager.getDeleteForegroundColor();
                    break;
                case MergedDescendentFileContentRow.ROWTYPE_INSERT:
                    this.color = ColorManager.getInsertForegroundColor();
                    break;
                default:
