...
public class OperationCompareRevisions extends OperationBaseClass {
     ...
     */
    OperationCompareRevisions(JTable fileTable, final String serverName, final String projectName, final String viewName, UserLocationProperties userLocationProperties) {
    ...
    void executeOperation() {
        if (getFileTable() != null) {
            try {
                ...
                QWinUtility.logProblem(Level.WARNING, "Caught exception in OperationCompareRevisions: " + e.getClass().toString() + ": " + e.getLocalizedMessage());
                QWinUtility.logProblem(Level.WARNING, Utility.expandStackTraceToString(e));
            }
        }
    }

    void processDialogResult(MergedInfoInterface mergedInfo, CompareRevisionsDialog compareRevisionsDialog) {
        try {
