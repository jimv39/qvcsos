...
# java -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5006 -Dapple.laf.useScreenMenuBar=true -Xmx512m -Xms512m -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -Xlog:gc:$QVCS_HOME/logs/qvcsos-client-gc.log -XX:+PrintGCDetails -XX:+PrintGCDateStamps -jar qvcse-gui-$QVCS_VERSION.jar
# java -Dapple.laf.useScreenMenuBar=true -Xmx512m -Xms512m -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -Xlog:gc:$QVCS_HOME/logs/qvcsos-client-gc.log -jar qvcse-gui-$QVCS_VERSION.jar "$QVCS_HOME"
cd ..
