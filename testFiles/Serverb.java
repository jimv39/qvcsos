...
//
// $Filename: /temp/QVCSTestFiles/Server.java $
//     $Date: Saturday, June 07, 2008 9:26:33 PM $
// $Revision: 1.91 $
//
// $Log5: REMOTE//Test Project///Server.java $
//
//    QVCS Enterprise Server.  A non-GUI QVCS Enterprise Server.
//
//  Revision 1.91 Author: JimVoris Date: Saturday, June 07, 2008 9:26:33 PM
//    CS9999 Code cleanup to make NetBeans happy.
//
//  Revision 1.90 Author: JimVoris Date: Thursday, February 21, 2008 8:14:35 PM
//    CS0097 Terminate connection to client when we haven't heard from the client for awhile.
//
//  Revision 1.89 Author: JimVoris Date: Wednesday, February 20, 2008 7:42:43 PM
//    CS0097 Enable keep alive on sockets to client so we have a chance to detect that
//    the client connection is gone.
//
//  Revision 1.88 Author: JimVoris Date: Thursday, October 04, 2007 3:17:24 PM
//    CS9999 Fix logoff for vanilla transport so we don't miss an IDE logoff.
//
//  Revision 1.87 Author: JimVoris Date: Wednesday, October 03, 2007 11:58:17 PM
//    CS9999 Scan files a 2nd time so that we're sure to assign fileID's to the files.
//
//  $Endlog$
//
// $Copyright � 2002-2008 Your Company, Inc.  ALL RIGHTS RESERVED WORLDWIDE. $
...
public class QVCSEnterpriseServer
{
    ...
    private static final String m_Version = "$Label: MULTIPLE LABELS $";

    static final int DEFAULT_VANILLA_LISTEN_PORT    = 10887;
    ...
    Vector<Runnable> m_Threads = new Vector<Runnable>();
    Vector<Runnable> m_VanillaThreads = new Vector<Runnable>();
    int m_ThreadCount = 1;
    ...
    static List<ServerResponseFactoryInterface>  connectedUsersCollection = Collections.synchronizedList(new ArrayList<ServerResponseFactoryInterface>());
    ...
    private void startServer()
    {
        ...
        {
            m_SecurePort = DEFAULT_ADMIN_LISTEN_PORT;
        }
        ...
        if (FileIDManager.getInstance().getScanRequiredFlag())
        {
            // We need to initialize things, and upgrade from a previous release...
            int maximumFileID = scanForFileID();
            FileIDManager.getInstance().setMaximumFileID(maximumFileID);
            FileIDManager.getInstance().setScanRequiredFlag(false);

            // Scan the files again... this time, fileID's will get assigned to
            // any file that is missing a fileID.
            maximumFileID = scanForFileID();

            // Scan for the maximum directory ID. Note that directory ID's are
     ...
     * for client connections... The goal is to scan all existing projects for
     * the maximum fileID. We do <i>not</i> use the ArchiveDirManager class here
     * as that is heavier-weight than what we want/need to do here.
     */
    private int scanForFileID()
    {
        m_logger.log(Level.INFO, "QVCSEnterpriseServer: scanning for maximum fileID.");
        ...
        try
        {
            ...
            for (int i = 0; i < projectPropertiesList.length; i++)
            {
                ...
                int projectMaximumFileID = scanDirectoryTreeForFileID(projectBaseDirectory);
                if (projectMaximumFileID > maximumFileID)
                {
                    maximumFileID = projectMaximumFileID;
                }
            }
        }
        catch (Exception e)
    ...
    /** For use by the scanForFileID() method only!!  Scan the given directory
     * and any sub-directories for the maximum fileID.
     */
    private int scanDirectoryTreeForFileID(File directory)
    {
        int maximumFileID = -1;
        ...
        if (fileList != null)
        {
            for (int i = 0; i < fileList.length; i++)
            {
                ...
                if (fileList[i].isDirectory())
                {
                    ...
                    int maxForSubDirectory = scanDirectoryTreeForFileID(fileList[i]);
                    if (maxForSubDirectory > maximumFileID)
                    {
                        maximumFileID = maxForSubDirectory;
                    }
                    continue;
                }
                ...
                {
                    int maxForLogFile = logfile.getFileID();
                    if (maxForLogFile > maximumFileID)
                    {
                        maximumFileID = maxForLogFile;
                    }
                }
                else
                {
    ...
    /** This method is called <i>before</i> the server opens any ports to listen
     * for client connections... The goal is to scan all existing projects for
     * their maximum directoryID.
     */
    private void scanForDirectoryID()
    {
        m_logger.log(Level.INFO, "QVCSEnterpriseServer: scanning for maximum directoryID.");
        ...
        try
        {
            ...
            for (int i = 0; i < projectPropertiesList.length; i++)
            {
                String archiveLocation = projectPropertiesList[i].getArchiveLocation();
                File projectBaseDirectory = new File(archiveLocation);
                int projectMaximumDirectoryID = scanDirectoryTreeForMaximumDirectoryID(projectBaseDirectory);
                DirectoryIDManager.getInstance().setMaximumDirectoryID(projectPropertiesList[i].getProjectName(), projectMaximumDirectoryID);
            }
        }
        catch (Exception e)
        {
    ...
    /** For use by the scanForDirectoryID() method only!!  Scan the given directory
     * and any sub-directories for the maximum directoryID.
     */
    private int scanDirectoryTreeForMaximumDirectoryID(File directory)
    {
        int maximumDirectoryID = -1;
        ...
        if (fileList != null)
        {
            for (int i = 0; i < fileList.length; i++)
            {
                ...
                {
                    // Recurse through the directory tree...
                    int maxForSubDirectory = scanDirectoryTreeForMaximumDirectoryID(fileList[i]);
                    if (maxForSubDirectory > maximumDirectoryID)
                    {
                        maximumDirectoryID = maxForSubDirectory;
                    }
                    continue;
                }
                if (fileList[i].getName().compareToIgnoreCase(QVCSConstants.QVCS_DIRECTORYID_FILENAME) == 0)
                {
                    try
                    {
                        DataInputStream dataInputStream = new DataInputStream(new FileInputStream(fileList[i]));
                        int fileID = dataInputStream.readInt();
                        dataInputStream.close();
                        if (fileID > maximumDirectoryID)
                        {
                            maximumDirectoryID = fileID;
                        }
                    }
                    catch (Exception e)
                    {
    ...
    private void deleteExistingDirectoryContentsObjects()
    {
        try
        {
            ...
            for (int i = 0; i < projectPropertiesList.length; i++)
            {
                ...
                String fullArchiveDirectoryName = directoryFile.getCanonicalPath();
