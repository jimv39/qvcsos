 ...
 * @author Jim Voris.
 */
public final class RepairCommonTime implements java.io.Serializable {

    private static final long serialVersionUID = -7285763794565705969L;

    /**
     * The internal long value for the time_t value, converted to the Java
     * representation.
     */
    private final RepairCommon32Long timeValue;
    private static final long MILLISECONDS_PER_SECOND = 1000L;

    /**
     * Default constructor. Default the time to now.
     */
    public RepairCommonTime() {
        Date now = new Date();
        this.timeValue = new RepairCommon32Long((int) (now.getTime() / MILLISECONDS_PER_SECOND));
    }

    /**
     * Constructor that to initialize the time using the given number of seconds
     * past the epoch.
     *
     * @param value new time value (second past the epoch).
     */
    public RepairCommonTime(int value) {
        this.timeValue = new RepairCommon32Long(value);
    }

    /**
     * Return a long value that represents the common time -- i.e. the number of
    ...
    /**
     * Read a time_t value from the random access file in the byte order native
     * to a Windows/Intel time_t value.
     *
     * @param inStream The random access stream to read from. Note that we
     * should be positioned at the bytes we want to read -- ie. this object
     * doesn't seek in the file at all.
    ...
    /**
     * Write the time out to the data stream in a byte order compatible with
     * Windows byte order for a C++ time_t value.
     *
