 ...
 * @author Jim Voris
 */
public class ServerResponsePromotionSimple extends AbstractServerResponsePromoteFile {
    private static final Logger LOGGER = LoggerFactory.getLogger(ServerResponsePromotionSimple.class);

    @Override
    public void updateDirManagerProxy(ArchiveDirManagerProxy directoryManagerProxy) {
        String message = String.format("ServerResponsePromotionSimple Promoted file: [%s::%s/%s] to [%s::%s/%s]", getMergedInfoSyncBranchName(), getMergedInfoSyncAppendedPath(),
    ...
    public ResponseOperationType getOperationType() {
        return ResponseOperationType.SR_PROMOTE_FILE_SIMPLE;
    }
