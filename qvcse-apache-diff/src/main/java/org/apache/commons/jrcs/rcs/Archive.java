 ...
 *
 * @version $Id: Archive.java,v 1.10 2004/02/28 03:35:36 bayard Exp $
...
//    private static final KeywordsFormat FORMATTER = new KeywordsFormat();
            ...
            throws PatchFailedException
    {
        ...
        for (int i = 0; i < text.length; i++)
        {
//            result[i] = FORMATTER.update(text[i].toString(), revisionInfo);   // TODO $Comment: get rid of call to KeywordsFormat.update. $
            result[i] = text[i].toString();         // TODO $Comment: Add line to replace keyword expansion with non-expanded text. $
        }
        return result;
    }
            ...
            throws PatchFailedException
    {
        ...
        for (int i = 0; i < text.length; i++)
        {
//            result[i] = FORMATTER.reset(text[i].toString());      // TODO $Comment: comment out call to reset keywords $
            result[i] = text[i].toString();     // TODO $Comment: replace resetting of passwords with original string. $
        }
        return result;
    }
