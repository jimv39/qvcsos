...
//   $Header: RoleManagerTest.java Revision:1.3 Wednesday, March 21, 2012 10:31:06 PM JimVoris $
...
public class RoleManagerTest {
    ...
    public void testAddUserRole() {
        ...
        if (RoleManager.getRoleManager().addUserRole(adminRoleType, TestHelper.getTestProjectName(), "JoeSmith", RoleManager.PROJECT_ADMIN_ROLE)) {
            AtomicReference<String> information = new AtomicReference<String>();
            if (RolePrivilegesManager.getInstance().isUserPrivileged(TestHelper.getTestProjectName(), "JoeSmith", RolePrivilegesManager.ADD_DIRECTORY, information)) {
                if (RoleManager.getRoleManager().addUserRole("JoeSmith", TestHelper.getTestProjectName(), "JaneSmith", RoleManager.READER_ROLE)) {
                    if (RolePrivilegesManager.getInstance().isUserPrivileged(TestHelper.getTestProjectName(), "JaneSmith", RolePrivilegesManager.GET, information)) {
                        if (RoleManager.getRoleManager().addUserRole("JoeSmith", TestHelper.getTestProjectName(), "JeffSmith", RoleManager.WRITER_ROLE)) {
                            if (RolePrivilegesManager.getInstance().isUserPrivileged(TestHelper.getTestProjectName(), "JeffSmith", RolePrivilegesManager.CHECK_IN, information)) {
