 ...
 * @author jimv
 */
public class LegacyDefaultCompressor implements LegacyCompressor {

    // Create our logger object
    private static final Logger LOGGER = LoggerFactory.getLogger(LegacyDefaultCompressor.class);
     ...
     * Default constructor.
     */
    public LegacyDefaultCompressor() {
    }

    /**
     * Compress the input buffer. Return true if we compressed the data, false
     * otherwise. If compression is successful, the outputBuffer will contain
     * the compressed result. The compressed result includes a
     * RevisionCompressionHeader as the first 10 bytes.
     *
     * Note that this method is not re-entrant, so we have to synchronize it --
     * it uses some object level variables that make it unworkable for it to be
     ...
     *
     * @param inputBufferLocal compressed input buffer that has a
     * RevisionCompressionHeader at its beginning.
    ...
    @Override
    public byte[] expand(LegacyRevisionCompressionHeader compressionHeader, byte[] inputBufferLocal) {
        int controlbits = 0;
        int control = 0;
        int srcIndex;
        int dstIndex;
        ...
        dstIndex = 0;
        srcIndex = LegacyRevisionCompressionHeader.getHeaderSize();
